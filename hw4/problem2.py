...
def compute_eigen_faces(X, k=20):
    ...
    # image_arrays = [reshape_to_image(image) for image in np.split(X, r)]

    # Xp, P_images = zip(*[p1.PCA(X_img, k) for X_img in image_arrays])

    Xp, P = map(np.real_if_close, p1.PCA(X, k))

    P_images = [reshape_to_image(image) for image in np.split(P.T, k)]
    #########################################
    return P_images, Xp
...
def load_dataset():
    ...
    response = fetch_olivetti_faces()

    X = np.matrix(response.get('data'))

    # the images
    images = response.get('images')

    # the label to predict is the id of the person
    y = np.matrix(response.get('target')).reshape((-1, 1))
